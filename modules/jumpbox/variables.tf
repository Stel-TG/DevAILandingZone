...
variable "subnet_id"            { type = string; description = "Subnet ID for jump box NIC (jump-box subnet)" }
variable "vm_size"              { type = string; default = "Standard_B2ms" }
variable "admin_username"       { type = string; default = "azureuser" }
variable "admin_ssh_public_key" { type = string; description = "SSH public key for admin access. Generate with: ssh-keygen -t rsa -b 4096" }
